...
export class DiagramComponent implements AfterViewInit {
  ...
  init() {
    ...
    this.diagram = new go.Diagram(
      'myDiagramDiv', // must name or refer to the DIV HTML element
      {
        grid: $(
          ...
          $(go.Shape, 'LineH', { stroke: 'gray', strokeWidth: 0, interval: 10 }),
    ...
    var nodeSelectionAdornmentTemplate = $(
      go.Adornment,
      'Auto',
      $(go.Shape, { fill: null, stroke: 'deepskyblue', strokeWidth: 3, strokeDashArray: [4, 2] }),
      $(go.Placeholder),
    );
    ...
    function activeHandler(e: any, obj: any) {
      ...
      if (data && typeof data.isActive === 'boolean') {
        ...
        node.diagram.model.commit((m: any) => m.set(data, 'clickCount', !data.isActive), 'status');
      }
    }
    // node template
      ...
      'myPaletteDiv', // must name or refer to the DIV HTML element
      {
        ...
        nodeTemplate: $(
          ...
          $(
            ...
            'Auto',
            { name: 'PANEL' },
            new go.Binding('desiredSize', 'sizeb', go.Size.parse).makeTwoWay(go.Size.stringify),
            $(
              go.Shape,
              'Rectangle', // default figure
              {
            ...
            $(
              go.Picture,
              { margin: 8, visible: true, desiredSize: new go.Size(50, 30), background: '#F4F4F4' },
  ...
  save() {
    ...
    (document.getElementById('mySavedModel') as HTMLInputElement)!.value = this.diagram.model.toJson();
    this.diagram.isModified = false;
  }
  load() {
    const json = JSON.parse((document.getElementById('mySavedModel') as HTMLInputElement).value);
